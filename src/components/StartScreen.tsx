...
export default function StartScreen({ onStart }: StartScreenProps) {
  ...
  return (
    <div className="flex h-full flex-col items-center justify-center gap-4 bg-cover bg-center bg-[url('/night-sky.webp')] font-mono text-white">
      <h1 className="text-5xl tracking-widest text-white">BLASTER</h1>
      <button onClick={onStart} className={buttonClass}>
        Start Game
      </button>
      <button onClick={error ? () => setError(null) : handleExit} className={buttonClass}>
        Exit Game
      </button>
